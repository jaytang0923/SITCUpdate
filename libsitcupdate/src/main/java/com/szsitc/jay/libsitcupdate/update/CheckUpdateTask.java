...
class CheckUpdateTask extends AsyncTask<Void, Void, String> {
    ...
    //this two just use for firmware update.
    private boolean isAPK;
    private int fwver;

    CheckUpdateTask(Context context, int type, boolean showProgressDialog ,boolean isapk,int fwver) {
    ...
    private void parseJson(String result) {
        try {
            ...
            Log.d("SITC",String.format("%s version check Current %d ,Server %d",isAPK?"APK":"Firmware",versionCode,apkCode));
            if (apkCode > versionCode) {
                if (mType == Constants.TYPE_NOTIFICATION) {
                    ...
                    showDialog(mContext, updateMessage, apkUrl ,isAPK ,apkCode);
                }
     ...
     * Show dialog
     */
    private void showDialog(Context context, String content, String apkUrl ,boolean isAPK,int apkCode) {
        UpdateDialog.show(context, content, apkUrl , isAPK,apkCode);
    }
